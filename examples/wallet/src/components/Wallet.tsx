...
import { claimNft, getAllOwnedNfts, getOwnedNftsInCollection, NftDetails } from '@nft-portal/wallet';
...
const Wallet = () => {
  ...
  useEffect(() => {
    const fetchNfts = async () => {
      ...
      const nfts = await getAllOwnedNfts(agent, {nftCollectionRegistryCid: PORTAL_STAGING_NFT_COLLECTION_REGISTRY});
      setAllNfts(nfts);
    };
    const fetchNftsInCollection = async () => {
      const nfts = await getOwnedNftsInCollection(agent, PORTAL_STAGING_NFT_COLLECTION);
  ...
  return (
    <div >
      <p>
        Total number of NFTs owned in Portal staging environment: {allNfts.length}
      </p>
      <p>
